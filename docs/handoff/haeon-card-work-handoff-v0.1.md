...
공격 콘솔은 <code>http://haeon-attack.localhost:8090/</code> 또는 <code>http://127.0.0.1:8094/</code>에서 연다. 콘솔은 해온카드 정상 진단, Before 공격 흐름, 대응, After 차단 검증만 실행하며, 북웨이브·PG 메뉴는 이후 독립 시나리오를 위한 자리다.

팀장이 만든 북웨이브·Mock PG 구현을 합칠 때는 현재 해온카드 작업 폴더를 변경하지 않고 전용 worktree를 사용한다. 기준 브랜치·커밋, 폴더 소유, Compose·환경 변수 경계, 통합 회귀 절차는 [통합 작업 가이드](bookwave-pg-haeon-integration-guide-v0.1.md)를 따른다.

## 서비스 경계

| 구성 | 주소·포트 | 역할 |
|---|---|---|
| 공격 콘솔 | <code>haeon-attack.localhost:8090</code>, <code>127.0.0.1:8094</code> | 해온카드 시나리오 실행·단계·이벤트 로그 |
...
촬영의 주 조작은 공격 콘솔에서 수행한다. 아래 명령행 도구는 재현 가능한 검증과 증거 보관용으로 사용한다.
