...
| 용도 | 주소 |
|---|---|
| 시연 터미널 | http://haeon-attack.localhost:8090/demo-terminal.html |
...
시연 터미널은 기존 운영 콘솔과 별도인 촬영 화면이다. 하나의 주 버튼이 `시연 시작 · 기준선 확인` → `다음 단계 · 공격자 Before` → `다음 단계 · 방어자 대응 / After` → `다음 단계 · 회원 보호`로 바뀐다. 각 단계의 타이핑 로그가 끝난 뒤 발표자가 다음 버튼을 눌러 장면을 진행한다. 화면에는 실제 실행 단계·상태·건수·수신 결과만 표시되며, 제어 토큰·가맹점 자격증명·세션 원문은 표시하거나 브라우저에 전달하지 않는다.

재생 시작 전에는 지원 서비스 연결 상태와 대응 대기 중인 Before 실행이 없는지를 확인한다. 이전 Before 실행이 남아 있으면 기존 운영 콘솔에서 대응 또는 초기화를 완료한 뒤 다시 재생한다. 각 다음 장면은 이전 단계가 실제 완료 상태로 확인된 뒤에만 표시된다.
